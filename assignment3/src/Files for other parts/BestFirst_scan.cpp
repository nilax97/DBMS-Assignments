    ...
    vector<knn> knn_search(point x, int k)
    {
        ...
        while(can_heap.size()>0)
        {
            ...
            if(popped_node->left!=NULL)
              {
                ...
                can_heap.push_back(MBR(dis_left, popped_node->left));
                make_heap(can_heap.begin(), can_heap.end(), &compare_MBR);
              }
              if(popped_node->right!=NULL)
              {
                double dis_right = distance_MBR(x,popped_node->right);
                can_heap.push_back(MBR(dis_right, popped_node->right));
                make_heap(can_heap.begin(), can_heap.end(), &compare_MBR);
              }
            }
            else
            {
              ...
              if(distance_MBR(x,popped_node)>max_dist)
                break;
